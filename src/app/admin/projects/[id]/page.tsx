...
`;
  ...
  const handlePublish = async () => {
    ...
    if (!title.trim() || !slug.trim() || !num.trim() || !description.trim() || !content.trim() || !githubUrl.trim() || !icon.trim()) {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-brand-bg flex items-center justify-center font-mono font-bold text-brand-ink text-sm">
        <motion.div animate={{ opacity: [1, 0] }} transition={{ duration: 0.8, repeat: Infinity }} className="w-3 h-3 bg-brand-red mr-3" />
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#1c1c1c] px-6 lg:px-12 h-16 flex items-center justify-between border-b-4 border-[#1c1c1c]">
        ...
        <div className="flex items-center gap-6">
          ...
          >
            ...
            {isPublishing ? 'SAVING...' : 'UPDATE_PROJECT'}
          </button>
        </div>
      </nav>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    value={slug}
                    disabled
                    className="w-full bg-brand-bg border-2 border-brand-ink p-3 text-sm font-mono outline-none opacity-60 cursor-not-allowed"
